...
async def fetch_chat_session(session_id: str) -> dict:
    """
    Returns the chat_session row.
    Called when a WebSocket connection opens; caller only needs patient_id,
    langgraph_thread_id, and agent_state — no join required.
    """
    def _query():
        return (
            _supabase()
            .table("chat_sessions")
            .select("id, patient_id, langgraph_thread_id, status, agent_state, token_count")
            .eq("id", session_id)
            .single()
            .execute()
        )
...
async def insert_pre_diagnostic(
    ...
    # Generate UUID client-side so we can return it regardless of supabase-py response.
    # Also set updated_at explicitly — Prisma's @updatedAt has no server-side DEFAULT,
    # so direct REST inserts must provide it or hit a NOT NULL violation.
